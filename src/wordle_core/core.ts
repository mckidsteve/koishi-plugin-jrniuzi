...
import { Argv, Command, Context, Element, h, Plugin, Session, Schema,Logger } from 'koishi'
...
return class {
    ...
    constructor(
        ...
        command.action(async (argv, word) => {
            ...
            return session?.text('wordle.messages.game-ended', [state.currentWord.join('')])
        }
        ...
        if (state?.state === Wordle.GameState.Active) {
            if (word) {
                ...
                if (result.type === 'correct') {
                    ...
                    await session.send(session?.text('wordle.messages.guess-incorrect', [variation.guessCount - state.guessedCount]))
                }
            }
            else {
            ...
            const logger = new Logger('wordle-logger')
            logger.info(currentWord)
            sessionState.set(`${session.guildId}.${session.channelId}`, { state: Wordle.GameState.Active, currentWord , guessedWords: [], guessedCount: 0})
